...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k , v, mask):
        ...
        query = query.view(q.shape[0],query.shape[1], self.n_heads, self.d_k).transpose(1,2)
        key = key.view(q.shape[0],key.shape[1], self.n_heads, self.d_k).transpose(1,2)
        value = value.view(q.shape[0],value.shape[1], self.n_heads, self.d_k).transpose(1,2)
